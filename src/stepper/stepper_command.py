...
ParamsType = TypeVar("ParamsType", bound="HasBytes")
ValueType = TypeVar("ValueType", int, bytes, float)
GroupSettingType = TypeVar("GroupSettingType", StoreFlag, SyncFlag)
...
class Command(ABC):
    ...
    def _command_body(self) -> bytes:
        ...
        body = bytes([self.addr, self._code])
        if self.params:
    ...
    def __repr__(self) -> str:
        ...
        return f"{self.__class__.__name__}({self.addr}, {self.params}, {self.setting})"
